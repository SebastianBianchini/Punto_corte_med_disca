...
```
 ...
 En este caso, antes de buscar el punto de corte óptimo, buscamos el precio unitario de cada medicamento en el último mes que aparece y lo tomamos como input de la función. Aquí no pisamos los valores, dejamos tales como venían, solamente tomamos como puntos de cortes los valores del último mes que aparecen.
...
# para cada medicamento, busco el ultimo mes y para ese ultimo mes me quedo con el precio unitario de farmacia
# me quedo con el maximo porque hay mas de un precio de farmacia en el mismo mes
precios_ultimo_mes <- df_med %>% 
                        dplyr::filter(ESTA_EN_MAC == 1) %>%
                        dplyr::group_by(CODIGO_DEFINITIVO,DNUM_NUMERO) %>% 
...
df_med_MAC <- df_med %>% 
          ...
          dplyr::mutate(NUM_IMPORTE_PAGO_OSDE_UNITARIO_ACT = max(NUM_IMPORTE_PAGO_OSDE_UNITARIO_ACT)) %>% 
          dplyr::ungroup()

tic()
# Calculo el umbral optimo, mediante la funcion, pasando como parametros el vector de precios unitarios unicos de farmacia correspondientes al ultimo mes en que aparece cada medicamento
data_umbral_optimo <- umbral_optimo(df = df_med_MAC,precios_unicos = sort(unique(precios_ultimo_mes$NUM_IMPORTE_PAGO_OSDE_UNITARIO_ACT_ultimo_mes)))
