...
```{r}
# 
# # Definir periodo de acumulación
# inicio <- 202401
# fin <- 202412
# 
# # Ordenar por ANOMES
...
# 
# # Filtrar los ANOMES dentro del rango deseado
# df_inflacion$EN_RANGO <- df_inflacion$ANOMES >= inicio & df_inflacion$ANOMES <= fin
# 
# # Crear acumulados por columna
# acumulado_por_columna <- function(var) {
#   # Inicializar la columna de acumulado con NA
#   acumulado <- rep(NA, nrow(df_inflacion))
#   
#   # Solo trabajar dentro del rango
#   idx <- which(df_inflacion$EN_RANGO)
#   
#   if (length(idx) > 0) {
#     factores <- 1 + df_inflacion[[var]][idx]
#     acumulados <- cumprod(factores)
#     acumulado[idx] <- acumulados - 1
#   }
#   
#   return(acumulado)
# }
# 
# # Aplicar para cada una de las columnas
# df_inflacion$ACUM_MDM_OSDE_PROM <- acumulado_por_columna("INFL_MDM_OSDE_PROM")
# df_inflacion$ACUM_MDM_CANASTA <- acumulado_por_columna("INFL_MDM_CANASTA")
# df_inflacion$ACUM_MDM_ALTO_COSTO <- acumulado_por_columna("INFL_MDM_ALTO_COSTO")
# 
# #eliminar la columna auxiliar
# df_inflacion$EN_RANGO <- NULL
# 
# # Ver resultado
# head(df_inflacion)
...
```
Asigno canasta y periodo segun corresponda 
```{r}
df_med <- data %>%  left_join(df_inflacion %>%  select(ANOMES, INDICE_PROM_HACIA_DIC, 
                                                       INDICE_CANASTA_HACIA_DIC, INDICE_ALTO_COSTO_HACIA_DIC), 
