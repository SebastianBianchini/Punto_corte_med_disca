...
deciles <- quantile(datos_ultimo_mes$NUM_IMPORTE_PAGO_OSDE_UNITARIO_ACT, 
                   probs = seq(0.1, 0.9, 0.1), na.rm = TRUE)

# Crear dataframe con información de deciles para el gráfico
deciles_df <- data.frame(
  decil = paste0("D", 1:9),
  valor = as.numeric(deciles),
  percentil = seq(10, 90, 10)
)
...
datos_ultimo_mes <- datos_ultimo_mes %>%
  mutate(
    DECIL = case_when(
      NUM_IMPORTE_PAGO_OSDE_UNITARIO_ACT <= deciles[1] ~ "D1",
      NUM_IMPORTE_PAGO_OSDE_UNITARIO_ACT <= deciles[2] ~ "D2",
      NUM_IMPORTE_PAGO_OSDE_UNITARIO_ACT <= deciles[3] ~ "D3",
      NUM_IMPORTE_PAGO_OSDE_UNITARIO_ACT <= deciles[4] ~ "D4",
      NUM_IMPORTE_PAGO_OSDE_UNITARIO_ACT <= deciles[5] ~ "D5",
      NUM_IMPORTE_PAGO_OSDE_UNITARIO_ACT <= deciles[6] ~ "D6",
      NUM_IMPORTE_PAGO_OSDE_UNITARIO_ACT <= deciles[7] ~ "D7",
      NUM_IMPORTE_PAGO_OSDE_UNITARIO_ACT <= deciles[8] ~ "D8",
      NUM_IMPORTE_PAGO_OSDE_UNITARIO_ACT <= deciles[9] ~ "D9",
      TRUE ~ "D10"
    )
  )
...
datos_ultimo_mes %>%   mutate(DIFERENCIA_IMPORTE_TOTAL =DIFERENCIA_IMPORTE_UNIT * NUM_CANTIDAD) %>%
                      group_by(DECIL,CANAL_CONVENIENTE) %>% 
                      summarise(CANTIDAD_ORDENES=n(), 
                                CANT_MED = sum(NUM_CANTIDAD),
                                SUM_DIFERENCIA_IMPORTE_TOTAL = abs(round(sum(DIFERENCIA_IMPORTE_TOTAL), 0))
                                ) %>%  arrange(DECIL,SUM_DIFERENCIA_IMPORTE_TOTAL,CANAL_CONVENIENTE ) %>%  View()
...
resumen_deciles <- datos_ultimo_mes %>%
...
resumen_deciles$DECIL <- factor(resumen_deciles$DECIL, levels = paste0("D", 1:10))

ggplot(resumen_deciles, aes(x = SUM_DIFERENCIA_IMPORTE_TOTAL, 
                             y = DECIL, 
                             fill = CANAL_CONVENIENTE)) +
  geom_col(position = "dodge") +
  labs(
    title = "Diferencia Total de Importe por Decil y Canal Conveniente",
